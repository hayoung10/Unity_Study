...
public class Enemy : LivingEntity {
    ...
    public float timeBetAttack = 0.5f; // 공격 간격 (=공격 사이의 시간 간격)
    ...
    private void Awake() {
        ...
        enemyRenderer = GetComponentInChildren<Renderer>(); // 렌더러 컴포넌트는 자식 게임 오브젝트에 있으므로
    }
    ...
    public void Setup(float newHealth, float newDamage, float newSpeed, Color skinColor) {
        ...
        health = newHealth;
        damage = newDamage;
        pathFinder.speed = newSpeed; // 내비메시 에이전트의 이동 속도 설정
        enemyRenderer.material.color = skinColor; // 렌더러가 사용 중인 머터리얼의 컬러를 변경, 외형 색이 변함
    }
    ...
    private IEnumerator UpdatePath() {
        ...
        while (!dead)
        {
            ...
            {
                pathFinder.isStopped = true; // 추적 대상 없음 : AI 이동 중지
    ...
    public override void Die() {
        ...
        pathFinder.isStopped = true; // AI 추적을 중지
        pathFinder.enabled = false; // 내비메시 컴포넌트 비활성화

        enemyAnimator.SetTrigger("Die"); // 사망 애니메이션 재생
        enemyAudioPlayer.PlayOneShot(deathSound); // 사망 효과음 재생
    }

    private void OnTriggerStay(Collider other) { // 트리거 충돌한 상대방 게임 오브젝트가 추적 대상이라면 공격 실행 
